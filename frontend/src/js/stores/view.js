...
var store = Reflux.createStore({
    ...
    */
    ...
    zero_out: function (ext, session) {         
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "http://localhost:8000/api/lesion?session=" + session, false);
        xhr.setRequestHeader('Content-type', 'application/json');
        xhr.onload = () => {
            if (xhr.status === 200) {
                console.log(xhr.responseText); 
                var current_view = this._data.view;
                var len = ext.end - ext.start;
                for (var i = 0; i < len; i++) {
                    this._data.data_bytes[ext.start + i] = 0;
                }
                var newview = this._data.data.fit(
                    new ds.Extent(current_view.start, current_view.end - 1)
                );
                ...
                this.trigger(this._data);
                var newview = this._data.data.fit(
                    new ds.Extent(current_view.start, current_view.end)
                );
                this._data.view = newview;
                var newfocus = newview.fit(this._data.focus);
                this._data.focus = newfocus;
        ...
        xhr.send(JSON.stringify({ "start_offset": ext.start, "end_offset": ext.end }));
    },
